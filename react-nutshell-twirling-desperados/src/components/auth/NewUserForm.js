...
const NewUserForm = (props) => {
    const [newUserInfo, setNewUserInfo] = useState({ username: "", email: "", password: "" })
    ...
    const handleRegistration = event => {
        ...
        LoginManager.getUsers().then(arrayOfUsers => {
            const filteredUsers = arrayOfUsers.filter(element => element.email === newUserInfo.email)
    ...
    return (
        <form onSubmit={handleRegistration}>
            <fieldset>
                ...
                <button disabled={isAvailable} type="submit">Add User</button>
            </fieldset>
        </form>
    )
